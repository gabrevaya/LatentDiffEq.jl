...
function train(; kws...)
    ...
    discriminator_img = Chain(Dense(input_dim, hidden_dim, relu),
                            Dense(hidden_dim, Int(hidden_dim/2), relu),
                            Dense(Int(hidden_dim/2), hidden_dim, relu),
...
# ###########################################################################
    ...
    @info "Start Discriminator-only Training of $(typeof(model_type))-net, for 10 epochs"
    @info "(reported loss from Discriminator only too)"
    for epoch = 1:10
        ...
        for x in loader_train
            ...
            val_set = Flux.unstack(first(loader_val), 2)
            t_val = range(0.f0, step=dt, length=length(val_set))
            val_loss = loss_batch_discriminator(generator, discriminator_img, discriminator_seq, λ, val_set |> device, t_val, af)
    ...
    ############################################################################
    ## Main train loop
    @info "Start Main Training of $(typeof(model_type))-net, total $epochs epochs"
    for epoch = 1:epochs
